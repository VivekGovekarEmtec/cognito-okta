from service import get_station_information
...
def lambda_handler(event, context):
    ...
    try:
        ...
        outlet_ids = get_query_param_value(query_string_params, "outlet_ids", str)
        product_id = get_query_param_value(query_string_params, "product_id", int)
        current_date = get_query_param_value(query_string_params, "current_date", str)
        lang = get_query_param_value(query_string_params, "lang", str)
        response_data = get_station_information(outlet_ids=outlet_ids, product_id=product_id, current_date=current_date,
                                                lang=lang)
        log.debug("Received response from get_station_information Service")
