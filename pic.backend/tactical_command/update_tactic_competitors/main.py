from service import update_tactic_competitors_for_outlets
...
log = Log().get_logger_service("Update tactic competitors")
...
def lambda_handler(event, context):
    ...
    try:
        request = event["body"]
        parsed_payload: UpdateTacticCompetitorsForOutlets = parse(event=request, model=UpdateTacticCompetitorsForOutlets)
        response_data = update_tactic_competitors_for_outlets(parsed_payload)
        log.debug("Received response from update_tactic_competitors_for_outlets Service")
