from service import insert_price_change
...
log = Log().get_logger_service("Create price change")
...
def lambda_handler(event, context):
    ...
    try:
        request = event["body"]
        parsed_payload: CreatePriceChange = parse(event=request, model=CreatePriceChange)
        response_body = insert_price_change(price_change=parsed_payload)
        log.debug("Received response from insert_price_change Service")
