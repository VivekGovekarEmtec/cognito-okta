from service import site_price_on_hold_by_site_id_update
...
def lambda_handler(event, context):
    ...
    try:
        request = event["body"]
        parsed_payload: PriceOnHoldUpdate = parse(event=request, model=PriceOnHoldUpdate)
        response_data = site_price_on_hold_by_site_id_update(parsed_payload)
        log.debug("Received response from site_price_on_hold_by_site_id_update Service")
