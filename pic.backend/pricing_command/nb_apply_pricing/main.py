from service import apply_pricing_nb
...
def lambda_handler(event, context):
    ...
    try:
        request = event["body"]
        parsed_payload: NBApplyPricing = parse(event=request, model=NBApplyPricing)
        response_data = apply_pricing_nb(parsed_payload)
        log.debug("Received response from apply_pricing_nb Service")
