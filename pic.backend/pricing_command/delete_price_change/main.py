from service import delete_price_change
...
def lambda_handler(event, context):
    ...
    try:
        request = event["body"]
        parsed_payload: PriceChangeDeleteList = parse(event=request, model=PriceChangeDeleteList)
        response_data = delete_price_change(parsed_payload)
        log.debug("Received response from delete_price_change Service")
