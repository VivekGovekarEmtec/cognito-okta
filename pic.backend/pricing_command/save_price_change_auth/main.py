from service import price_change_auth_update
...
def lambda_handler(event, context):
    ...
    try:
        request = event["body"]
        parsed_payload: SiteProductPriceChangeUpdate = parse(event=request, model=SiteProductPriceChangeUpdate)
        response_data = price_change_auth_update(parsed_payload)
        log.debug("Received response from price_change_auth_update Service")
