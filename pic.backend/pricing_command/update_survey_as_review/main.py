from service import mark_survey_review_update_service
...
log = Log().get_logger_service("mark_survey_review_update_service")
...
def lambda_handler(event, context):
    ...
    try:
        request = event["body"]
        parsed_payload: MarkReview = parse(event=request, model=MarkReview)
        response_data = mark_survey_review_update_service(parsed_payload)
        log.debug("Received response from mark_survey_review_update_service Service")
