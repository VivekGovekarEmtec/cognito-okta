from service import insert_price_hold
...
def lambda_handler(event, context):
    ...
    try:
        request = event["body"]
        parsed_payload: CreatePriceOnHold = parse(event=request, model=CreatePriceOnHold)
        response_body = insert_price_hold(parsed_payload)
        log.debug("Received response from insert_price_hold Service")
