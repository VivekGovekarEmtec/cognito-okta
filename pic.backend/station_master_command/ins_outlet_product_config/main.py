...
from service import insert_outlet_product_configuration
...
def lambda_handler(event, context):
    ...
    try:
        request = event['body']
        parsed_payload: ProductConfiguration = parse(event=request,model=ProductConfiguration)
        if(event['path'] == "/master/product/saveNewOutletProductConfiguration"):
            new_product = insert_outlet_product_configuration(parsed_payload)
        else: 
            new_product = {
            "status_code": 404,
            "message": "Invalid api",
        ...
        log.debug("Received response from Outlet product configuaration service")
        ...
        return create_response(new_product)