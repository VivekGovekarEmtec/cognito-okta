...
from service import is_auto_clear_survey_toggle
...
log = Log().get_logger_service("Get pic-national-competitor-command-lambda-autoClearSurveyToggle")
...
def lambda_handler(event, context):
    ...
    try:
        ...
        if(event['path'] == "/competitors/association/autoClearSurveyToggle"):
            auto_clear_survey_toggle_response = is_auto_clear_survey_toggle(site_id, user_id)
        else: 
            auto_clear_survey_toggle_response = {
            "status_code": 404,
            "message": "Invalid api",
            "data": {}
        }
        log.debug("Received response from autoClearSurveyToggle service")
        ...
        return create_response(auto_clear_survey_toggle_response)