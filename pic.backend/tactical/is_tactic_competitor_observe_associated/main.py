from service import is_competitor_observe_associated
...
log = Log().get_logger_service("Is tactic competitors observe associated")
...
def lambda_handler(event, context):
    ...
    try:
        ...
        observe_type_id = get_query_param_value(query_string_params, "observe_type_id", int)

        response_data = is_competitor_observe_associated(site_id, kent_id, observe_type_id)

        log.debug("Received response from is_competitor_observe_associated Service")
