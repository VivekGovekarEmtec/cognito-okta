from service import get_facility_types
...
log = Log().get_logger_service("Get Facility Types")
...
def lambda_handler(event, context):
    ...
    if 'refresh_frequency' in event:
        refresh_cache_data(event['refresh_frequency'], 'facility_types_list')

    elif event['path']:
        log.append_keys(resource_path=event['path'])
        if event['path'] == "/common/master/references/facilityTypes":
            try:
                query_string_params = event["queryStringParameters"]
                language = get_query_param_value(query_string_params, "language", str)
                response_data = get_facility_types(language)
                log.debug("Received response from get_facility_types Service")
