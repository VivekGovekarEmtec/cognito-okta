from service import get_base_products
...
log = Log().get_logger_service("Get base products")
...
def lambda_handler(event, context):
    ...
    if 'refresh_frequency' in event:
        refresh_cache_data(event['refresh_frequency'], 'base_product_list')

    elif event['path']:
        log.append_keys(resource_path=event['path'])
        if event['path'] == "/common/competitors/surveyManagement/getBaseProducts":
            try:
                query_string_params = event["queryStringParameters"]
                language = get_query_param_value(query_string_params, "language", str)
                response_data = get_base_products(language)
                log.debug("Received response from get_base_products Service")
