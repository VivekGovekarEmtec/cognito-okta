from service import get_all_time_zones
...
log = Log().get_logger_service("Get all time zones")
...
def lambda_handler(event, context):
    ...
    if 'refresh_frequency' in event:
        refresh_cache_data(event['refresh_frequency'], 'time_zones_list')

    elif event['path']:
        log.append_keys(resource_path=event['path'])
        if event['path'] == "/common/master/references/getOutletTimeZones":
            try:
                query_string_params = event["queryStringParameters"]
                language = get_query_param_value(query_string_params, "language", str)
                response_data = get_all_time_zones(language)
                log.debug("Received response from get_all_time_zones Service")
