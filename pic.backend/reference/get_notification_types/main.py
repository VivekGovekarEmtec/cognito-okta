...
log = Log().get_logger_service("Get Notification Types")
...
def lambda_handler(event, context):
    ...
    if 'refresh_frequency' in event:
        refresh_cache_data(event['refresh_frequency'], 'notification_types')

    elif event['path']:
        log.append_keys(resource_path=event['path'])
        if event['path'] == "/common/master/product/getNotificationTypes":
            try:
                query_string_params = event["queryStringParameters"]
                language = get_query_param_value(query_string_params, "language", str)
                response_data = get_notification_types(language)
                log.debug("Received response from get_notification_types Service")
