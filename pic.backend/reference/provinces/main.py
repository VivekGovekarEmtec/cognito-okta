from service import get_province
...
log = Log().get_logger_service("Get Province")
...
def lambda_handler(event, context):
    ...
    if 'refresh_frequency' in event:
        refresh_cache_data(event['refresh_frequency'], 'province_list')

    elif event['path']:
        log.append_keys(resource_path=event['path'])
        if event['path'] == "/common/master/references/provinces":
            try:
                query_string_params = event["queryStringParameters"]
                language = get_query_param_value(query_string_params, "language", str)
                response_data = get_province(language)
                log.debug("Received response from get_province Service")
