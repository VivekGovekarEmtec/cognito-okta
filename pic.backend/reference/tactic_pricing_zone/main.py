from service import get_tactic_pricing_zone
...
log = Log().get_logger_service("Get tactical pricing zone")
...
def lambda_handler(event, context):
    ...
    if 'refresh_frequency' in event:
        refresh_cache_data(event['refresh_frequency'], 'tactic_pricing_zone_list')

    elif event['path']:
        log.append_keys(resource_path=event['path'])
        if event['path'] == "/common/master/references/tacticPricingZone":
            try:
                ...
                response_data = get_tactic_pricing_zone(product_id, language)
                log.debug("Received response from get_tactic_pricing_zone Service")
