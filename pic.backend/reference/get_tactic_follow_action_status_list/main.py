from service import get_tactic_follow_action_status
...
log = Log().get_logger_service("Get tactic follow action status list")
...
def lambda_handler(event, context):
    ...
    if 'refresh_frequency' in event:
        refresh_cache_data(event['refresh_frequency'], 'follow_action_status_list')

    elif event['path']:
        log.append_keys(resource_path=event['path'])
        if event['path'] == "/tactical/competitor/getTacticFollowActionStatusList":
            try:
                query_string_params = event["queryStringParameters"]
                language = get_query_param_value(query_string_params, "language", str)
                response_data = get_tactic_follow_action_status(language)
                log.debug("Received response from get_tactic_follow_action_status Service")
