from service import get_contact_types
...
log = Log().get_logger_service("Get Contact Types list")
...
def lambda_handler(event, context):
    ...
    if 'refresh_frequency' in event:
        refresh_cache_data(event['refresh_frequency'], 'contact_types_list')

    elif event['path']:
        log.append_keys(resource_path=event['path'])
        if event['path'] == "/common/master/contact/getContactTypes":
            try:
                query_string_params = event["queryStringParameters"]
                language = get_query_param_value(query_string_params, "language", str)
                response_data = get_contact_types(language)
                log.debug("Received response from get_contact_types Service")
