from service import get_outlet_selector
...
log = Log().get_logger_service("Get Outlet selector")
...
def lambda_handler(event, context):
    ...
    if 'refresh_frequency' in event:
        refresh_cache_data(event['refresh_frequency'], 'outlet_selector_list')

    elif event['path']:
        log.append_keys(resource_path=event['path'])
        if event['path'] == "/common/master/outlet/getOutletSelector":
            try:
                ...
                response_data = get_outlet_selector(user_id, language)
                log.debug("Received response from get_outlet_selector Service")
